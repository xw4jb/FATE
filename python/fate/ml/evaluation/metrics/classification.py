...
def _neg_pos_count(labels: np.ndarray, pos_label: int):
    pos_num = ((labels == pos_label) + 0).sum()
    neg_num = len(labels) - pos_num
    return pos_num, neg_num


def _sort_score_and_label(labels: np.ndarray, pred_scores: np.ndarray):
...
class KS(Metric):
    ...
    def fit(labels, pred_scores, pos_label=1, fixed_interval_threshold=True):
        ...
        confusion_mat = _ConfusionMatrix.compute(sorted_labels, sorted_scores, threshold, ret=['tp', 'fp'],
                                                pos_label=pos_label)
        pos_num, neg_num = _neg_pos_count(sorted_labels, pos_label=pos_label)
