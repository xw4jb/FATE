...
class HeteroLinrModuleGuest(HeteroModule):
    ...
    def fit(self, ctx: Context, train_data, validate_data=None) -> None:
        ...
        estimator.fit_single_model(ctx, train_data, validate_data, with_weight=with_weight)
...
class HeteroLinrEstimatorGuest(HeteroModule):
    ...
    def fit_single_model(self, ctx, train_data, validate_data=None, with_weight=False):
        ...
        if self.init_param.fit_intercept:
            ...
            coef_count += 1
        if self.w is None:
            w = initialize_param(self.init_param.method, coef_count,
                                 self.init_param.fit_inercept, self.init_param.const)
        # mock data
        ...
        for i, iter_ctx in ctx.range(self.start_iter, self.max_iter):
            ...
            for batch_ctx, (X, Y, weight) in iter_ctx.iter(batch_loader):
                ...
                d = Xw - Y
                loss = 1 / 2 / h * tensor.matmul(d.T, d)
                ...
                for Xw_h in batch_ctx.hosts.get("Xw_h"):
                    d += Xw_h
                    loss += 1 / h * tensor.matmul(Xw.T, Xw_h)
                if with_weight:
                    d = d * weight
    ...
    def restore(self, model):
        ...
        self.w = tensor.tensor(torch.tensor(model["w"]))
