...
class HeteroLinrModuleHost(HeteroModule):
    ...
    def fit(self, ctx: Context, train_data, validate_data=None) -> None:
        ...
        estimator.fit_single_model(ctx, encryptor, train_data, validate_data)
...
class HeteroLinrEstimatorHost(HeteroModule):
    ...
    def fit_single_model(self, ctx: Context, encryptor, train_data, validate_data=None) -> None:
        ...
        w = initialize_param(self.init_param.method, self.train_data.shape[1], False, self.init_param.const)
        for i, iter_ctx in ctx.range(self.start_iter, self.max_iter):
            ...
            for batch_ctx, X in iter_ctx.iter(batch_loader):
                ...
                g = self.optimizer.add_regular_to_grad(tensor.matmul(X.T, d), w)
