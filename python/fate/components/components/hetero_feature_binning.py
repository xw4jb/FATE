...
@cpn.parameter("method", type=str, default="quantile", optional=False,
...
"""def train_guest(ctx, train_data, train_output_data, output_model, method, n_bins, split_pt_dict,
                bin_col, bin_idx, category_col, category_idx, use_anonymous, transform_method,
                skip_metrics, local_only, error_rate, adjustment_factor):
    from fate.ml.feature_binning import HeteroBinningModuleGuest
    columns = train_data.schema.columns
    anonymous_columns = None
    if use_anonymous:
        anonymous_columns = train_data.schema.anonymous_columns
    ...
    with ctx.sub_ctx("train") as sub_ctx:
        ...
        model = binning.to_model()
        with output_model as model_writer:
            model_writer.write_model("feature_binning", model, metadata={})
...
def train_host(ctx, train_data, train_output_data, output_model, method, n_bins, split_pt_dict,
               bin_col, bin_idx, category_col, category_idx, use_anonymous, transform_method,
               skip_metrics, local_only, error_rate, adjustment_factor):
    from fate.ml.feature_binning import HeteroBinningModuleHost
    columns = train_data.schema.columns
    anonymous_columns = None
    if use_anonymous:
        anonymous_columns = train_data.schema.anonymous_columns
