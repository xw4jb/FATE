...
OPTIMIZER = {'adadelta', 'adagrad', 'adam', 'adamw', 'adamax'
             'asgd', 'nadam', 'radam' 'rmsprop', 'rprop', 'sgd'}


class LRScheduler():
    def __init__(self, lr_params, iters=0):
        self.lr_params = lr_params
        self.iters = iters
        self.lr_scheduler = None

    def init_scheduler(self, optimizer):
        self.lr_scheduler = lr_scheduler_factory(optimizer, self.lr_params)
...
class _Optimizer(object):
    def __init__(self, optim_params, iters=0):
        self.optim_params = optim_params
        self.iters = iters
        self.l2_penalty = True if optim_params.penalty == "l2" else False
        self.l1_penalty = True if optim_params.penalty == "l1" else False
    ...
    def init_optimizer(self, model_parameter_length):
        model_parameter = torch.nn.parameter.Parameter(torch.tensor([[0.0]] * model_parameter_length),
                                                       requires_grad=True)
        self.model_parameter = model_parameter
        self.optimizer = optimizer_factory([model_parameter], self.optim_params)
        # for regularization
    ...
    def load_state_dict(self, dict):
        self.optimizer.load_state_dict(dict)
...
def optimizer_factory(model_parameter, param):
    optimizer_type = param.method
...
def lr_scheduler_factory(optimizer, param):
    scheduler_method = param.method
    scheduler_params = param.scheduler_params
