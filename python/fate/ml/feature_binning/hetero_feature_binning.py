...
class HeteroBinningModuleGuest(Module):
    ...
    def compute_metrics(self, ctx: Context, binned_data):
        y_col = binned_data[binned_data.schema.label_name].to_tensor()
        self._bin_obj.compute_metrics(binned_data, y_col)
    ...
    def compute_federated_metrics(self, ctx: Context, binned_data):
        ...
        label_col = binned_data[binned_data.schema.label_name]
        encryptor.encrypt(label_col).to(ctx.hosts, "enc_y")
...
class StandardBinning(Module):
    ...
    def fit(self, ctx: Context, train_data, validate_data=None):
        ...
        if self.method == "quantile":
            ...
            split_pt_df = train_data.quantile(q=q,
                                               ...
                                               skip_none=True)
            split_pt_dict = {}
            for col in split_pt_df.columns:
    ...
    def is_monotonic(woe_array):
        ...
        is_increasing = np.all(woe_array[1:] > woe_array[:-1])
        is_decreasing = np.all(woe_array[1:] < woe_array[:-1])
    ...
    def compute_metrics_from_count(self, event_count_array, non_event_count_array,
        ...
        bin_woe = np.log(event_rate / non_event_rate)
    ...
    def compute_all_col_metrics(self, col_list, event_count_hist, non_event_count_hist):
        ...
        for col in col_list:
            col_summary = {}
            event_count_array = event_count_hist[col].to_numpy
            non_event_count_array = non_event_count_hist[col].to_numpy
            if total_event_count is None:
                total_event_count = max(np.sum(event_count_array), 1)
                total_non_event_count = max(np.sum(non_event_count_array), 1)

            event_rate, non_event_rate, bin_woe, bin_iv = self.compute_metrics_from_count(event_count_array,
                                                                                          non_event_count_array,
                                                                                          total_event_count,
                                                                                          total_non_event_count)
            col_summary["event_count"] = event_count_array
            col_summary["non_event_count"] = non_event_count_array
            col_summary["event_rate"] = event_rate
            col_summary["non_event_rate"] = non_event_rate
            col_summary["woe_array"] = bin_woe
            col_summary["iv_array"] = bin_iv
            col_summary["is_monotonic"] = StandardBinning.is_monotonic(bin_woe)
            col_summary["col_iv"] = bin_iv.sum()
            metrics_summary[col] = col_summary
            woe_dict[col] = bin_woe
    ...
    def compute_metrics(self, binned_data, label_col):
        ...
        bin_count_hist = binned_data.histogram2D(columns=to_compute_col, target=tensor([1] * binned_data.count()))
