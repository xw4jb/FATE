...
@cpn.parameter("early_stop", type=params.StringChoice(["weight_diff", "diff", "abs"]), default="diff",
...
@cpn.parameter(
    "optimizer", type=params.Optimizer(),
    default=params.Optimizer(method="sgd", lr=1e-2, penalty='l2', decay=0, decay_sqrt=False),
    desc="optimizer, select method from {'sgd', 'nesterov_momentum_sgd', 'adam', 'rmsprop', 'adagrad', 'sqn'} "
         "for list of configurable arguments, refer to params.Optimizer")
...
def train(
    ...
    if role.is_guest:
        train_guest(
            ...
            batch_size, optimizer, init_param
        )
    elif role.is_host:
        train_host(
            ctx, train_data, validate_data, train_output_data, output_model, max_iter,
        ...
        train_arbiter(ctx, max_iter, early_stop, tol, batch_size, optimizer, train_output_metric)
...
def train_host(ctx, train_data, validate_data, train_output_data, output_model, max_iter, batch_size, optimizer_param,
               init_param):
    ...
    with ctx.sub_ctx("train") as sub_ctx:
        module = HeteroLinrModuleHost(max_iter=max_iter, batch_size=batch_size, optimizer=optimizer, init_param=init_param)
        train_data = sub_ctx.reader(train_data).read_dataframe()
        if validate_data is not None:
            validate_data = sub_ctx.reader(validate_data).read_dataframe()
        module.fit(sub_ctx, train_data, validate_data)
...
def train_arbiter(ctx, max_iter, early_stop, tol, batch_size, optimizer_param, train_output_metric):
